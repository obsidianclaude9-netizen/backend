...
exports.excelExportService = exports.ExcelExportService = void 0;
...
class ExcelExportService {
    ...
    async addRevenueSheet(workbook, _options) {
        const sheet = workbook.addWorksheet('Revenue Analysis', {
            properties: { tabColor: { argb: 'FF4CAF50' } }
        });
        const monthlyData = await materialized_views_service_1.viewService.getMonthlyRevenue(12);
        sheet.columns = [
    ...
    async addTicketsSheet(workbook) {
        ...
        const sessionData = await materialized_views_service_1.viewService.getSessionPerformance();
        sheet.columns = [
    ...
    async addCustomersSheet(workbook) {
        const sheet = workbook.addWorksheet('Customer Segments', {
            properties: { tabColor: { argb: 'FFFF9800' } }
        });
        const segmentData = await materialized_views_service_1.viewService.getCustomerSegments();
        sheet.columns = [
    ...
    async addSessionsSheet(workbook) {
        const sheet = workbook.addWorksheet('Session Performance');
        const sessions = await materialized_views_service_1.viewService.getSessionPerformance();
        sheet.columns = [
    ...
    async addScansSheet(workbook, options) {
        ...
        const scanData = await materialized_views_service_1.viewService.getScanStats(startDate, endDate);
        sheet.columns = [
...
exports.excelExportService = new ExcelExportService();
