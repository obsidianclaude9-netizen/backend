...
exports.CustomerCampaignService = void 0;
...
class CustomerCampaignService {
     ...
     */
    async getCustomerGrowth(period) {
        ...
        const timeline = months.map(monthDate => {
            ...
            const newInMonth = customers.filter(c => (0, date_fns_1.format)(c.createdAt, 'yyyy-MM') === month).length;
            // Calculate cumulative total up to this month
            const totalUpToMonth = customers.filter(c => c.createdAt <= new Date(month + '-31')).length;
     ...
     */
    async getCustomerRetention(range) {
        const customers = await database_1.default.customer.findMany({
            where: {
     ...
     */
    async getCustomerSegments(range) {
        const customers = await database_1.default.customer.findMany({
            where: {
                orders: {
                    some: {
        ...
        for (const customer of customers) {
            ...
            const daysSinceLastPurchase = customer.lastPurchase
                ? (0, date_fns_1.differenceInDays)(new Date(), customer.lastPurchase)
                : 999;
            // Segmentation logic
     ...
     */
    async getMarketingMetrics(range) {
        const [emailStats, customerStats, revenueStats, campaignStats,] = await Promise.all([
    ...
    async calculateEmailStats(range) {
        const campaigns = await database_1.default.campaign.findMany({
            where: {
                status: 'SENT',
                sentAt: { gte: range.start, lte: range.end },
        ...
        const totalOpened = campaigns.reduce((sum, c) => sum + c.tracking.filter(t => t.opened).length, 0);
        const totalClicked = campaigns.reduce((sum, c) => sum + c.tracking.filter(t => t.clicked).length, 0);
        return {
    ...
    async getCustomerMetrics(range) {
        const customers = await database_1.default.customer.findMany({
            include: {
                orders: {
    ...
    async calculateLTV(range) {
        ...
        const customerCount = await database_1.default.customer.count({
            where: {
                orders: {
                    some: {
                        status: 'COMPLETED',
    ...
    async calculateRevenueMetrics(range, marketingCost) {
        const newCustomers = await database_1.default.customer.count({
    ...
    async getCampaignStats(range) {
        ...
        const totalSent = campaigns.reduce((sum, c) => sum + c.sentTo, 0);
        const totalConverted = campaigns.reduce((sum, c) => sum + c.tracking.filter(t => t.converted).length, 0);
        const conversionRate = totalSent > 0 ? (totalConverted / totalSent) * 100 : 0;
        // Previous period
        const prevRange = this.getPreviousPeriod(range);
    ...
    async calculateConversionRate(range) {
        const campaigns = await database_1.default.campaign.findMany({
            where: {
                status: 'SENT',
                sentAt: { gte: range.start, lte: range.end },
...
exports.CustomerCampaignService = CustomerCampaignService;
//# sourceMappingURL=customer-campaign.service.js.map